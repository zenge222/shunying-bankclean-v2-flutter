...
class CheckEventReport extends StatefulWidget {
  final int id;

  const CheckEventReport({Key key, this.id}) : super(key: key);

  @override
  _CheckEventReportState createState() => _CheckEventReportState();
}

/// 巡检+？
class _CheckEventReportState extends State<CheckEventReport>
    with ComPageWidget {
  ...
  Future _submitReport() async {
    if (_fromImages.length == 0) return showToast('请上传照片');
    if (content == "") return showToast('请填写内容');
    Map params = new Map();
    params['id'] = widget.id;
    params['resultContent'] = content;
    params['resultImages'] = _fromImages.join(',');
    Api.submitEditReport(map: params).then((res) {
      if (res.code == 1) {
//        Application.router.pop(context);
