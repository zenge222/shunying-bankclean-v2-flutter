...
class CheckSchedulingEdit extends StatefulWidget {
  final int id;
  final dateStr;

  const CheckSchedulingEdit({Key key, this.id, this.dateStr}) : super(key: key);

  @override
  _CheckSchedulingEditState createState() => _CheckSchedulingEditState();
}

class _CheckSchedulingEditState extends State<CheckSchedulingEdit>
    with ComPageWidget {
...
//                                      maxValue: DateTime(today.year + 1,today.month,today.day),
                                    ...
                                    onTap: () {
                                      Navigator.push(
                                        ...
                                        if (data != null) {
                                          print('改变数据');
                                          setState(() {
                                            resData.cleanerId = data.id;
                                            resData.cleanerName = data.name;
                                          });
                                        }
                                      });
                                    },
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                    ...
                                    onTap: () {
                                      Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                            builder: (context) =>
                                                DutyPublicSelect(
                                              id: resData.organizationBranchId,
                                            ),
                                          )).then((data) {
                                        if (data != null) {
                                          print('改变数据');
                                ...
                                Offstage(
                                  offstage:  false,//resData.id!=-1,//resData.title != '自定义',
                                  child: Column(
                                    children: <Widget>[
                                      Container(
                                        margin: EdgeInsets.only(
  ...
  Future _submitReport() async {
    if (resData.cleanerId == 0) {
      showToast('请选择人员');
    } else if (resData.id == 0) {
      showToast('请选择职责');
    } else {
      Map params = new Map();
      params['cleanerId'] = resData.cleanerId;
      params['taskRequestId'] = widget.id;
      params['dateStr'] = widget.dateStr;
      params['startTimeStr'] = resData.startTime;
      params['endTimeStr'] = resData.endTime;
      params['selectTaskId'] =  -1; //resData.title == "自定义" ? -1 : resData.id;
      params['taskId'] = widget.id;
      params['title'] = resData.title;
        if (taskList.length == 0) {
          showToast('请添加一条自定义');
        } else {
          Api.taskEditCommit(map: params, formData: taskList).then((res) {
            if (res.code == 1) {
