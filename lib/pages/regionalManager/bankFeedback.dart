...
class BankFeedback extends StatefulWidget {
  @override
  _BankFeedbackState createState() => _BankFeedbackState();
}

class _BankFeedbackState extends State<BankFeedback> with ComPageWidget {
  ...
  Future _submitReport() async {
    if (_contentController.text == '') return showToast('请填写意见内容');
//    if (_fromImages.length == 0) return showToast('请上传图片');
    Map params = new Map();
    params['content'] = _contentController.text;
    params['images'] = _fromImages.join(',');
    Api.adviceFeedbackCreate(map: params).then((res) {
      if (res.code == 1) {
        setState(() {
          _fromImages = [];
          _images = [];
